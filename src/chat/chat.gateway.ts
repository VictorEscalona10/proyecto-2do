...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinChat(
    ...
    try {
      ...
      const res = await this.chatService.joinChat(user, data.chatId);
  ...
  async handleSendMessage(
    ...
    try {
      ...
      const res = await this.chatService.sendMessage(user, data.chatId, data.text);
...
async handleGetMyChats(@ConnectedSocket() client: Socket) {
  ...
  async handleGetChatMessages(
    ...
    try {
      ...
      const res = await this.chatService.getChatMessages(user, data.chatId);
...
async handleCloseChat(
  ...
  try {
    ...
    const res = await this.chatService.closeChat(user, data.chatId);
